...
8. E-mail receipts are deferred by owner decision. This is not a SATIM waiver. The demo hides the e-mail form when no mail transport is configured; print and PDF receipts remain. Setting `SMTP_URL` later turns it back on with no code change. Confirm the qualifier accepts the deferral before treating the certification checklist as complete.
