...
- Reconciliation of abandoned orders runs from the Vercel cron in `vercel.json` (every six hours; Hobby plans run crons at most once a day, which Vercel adjusts automatically) and on demand through the admin endpoint. Customers who return normally are acknowledged immediately either way.
